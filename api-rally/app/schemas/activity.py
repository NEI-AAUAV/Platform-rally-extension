...
class TeamVsResult(BaseModel):
    ...
    completed: bool = Field(
        default=True,
        description="Whether the team completed the challenge (awards completion_points)",
    )
...
class GeneralResult(BaseModel):
    ...
    assigned_points: float = Field(
        ..., ge=0, description="Points assigned by staff within configured range"
    )
...
class TeamVsConfig(BaseModel):
    ...
    completion_points: int = Field(
        default=0, ge=0, description="Bonus points when team completes the challenge"
    )
    # Outcome points
