...
class CRUDRallyGuideAssignment(CRUDBase[RallyGuideAssignment, RallyGuideAssignmentCreate, RallyGuideAssignmentUpdate]):
    ...
    async def get_by_checkpoint_id(self, db: AsyncSession, checkpoint_id: int) -> Sequence[RallyGuideAssignment]:
        ...
        stmt = select(RallyGuideAssignment).where(RallyGuideAssignment.checkpoint_id == checkpoint_id)
    ...
    async def create_or_update(self, db: AsyncSession, *, user_id: int, checkpoint_id: Optional[int] = None) -> Optional[RallyGuideAssignment]:
