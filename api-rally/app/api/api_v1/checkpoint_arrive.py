...
class CheckpointArriveController:
    ...
    async def arrive_at_checkpoint(
        ...
        # advances. `auto_completed` answers "did this arrival complete the
        # post", so a repeat tap reports False.
        completes_post = await service.auto_complete_if_no_activities(team.team_id, checkpoint_id)
        auto_completed = completes_post and not already_registered
