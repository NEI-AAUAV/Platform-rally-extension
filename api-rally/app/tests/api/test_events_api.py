...
async def test_update_event(pg_session, pg_client):
...
async def test_set_current_event(pg_session, pg_client):
...
async def test_rotation_schedule_rejects_non_olympic_event(pg_session, pg_client):
...
async def test_rotation_schedule_rejects_when_no_teams_or_checkpoints(pg_session, pg_client):
    created = await rally_event.create(
...
async def test_rotation_schedule_generates_and_persists(pg_session, pg_client):
