...
class TestRallySettingsCRUD:
    ...
    async def test_get_or_create_settings_new(self, mock_db, mock_rally_settings_data):
        ...
        with patch.object(rally_settings, 'get_or_create') as mock_get_or_create:
            ...
            result = await rally_settings.get_or_create(mock_db)
    ...
    async def test_get_or_create_settings_existing(self, mock_db, mock_rally_settings_data):
    ...
    async def test_update_settings(self, mock_db, mock_rally_settings_data):
        ...
        with patch.object(rally_settings, 'update') as mock_update:
            ...
            result = await rally_settings.update(mock_db, db_obj=mock_rally_settings_data, obj_in=update_data)
...
class TestTeamCRUD:
    ...
    async def test_create_team(self, mock_db, mock_team_data):
        ...
        with patch.object(crud_team, 'create') as mock_create:
            ...
            result = await crud_team.create(mock_db, obj_in=team_data)
            
            assert result is not None
            assert result["name"] == "Test Team"
    
    async def test_get_team(self, mock_db, mock_team_data):
        """Test getting a team"""
        with patch.object(crud_team, 'get') as mock_get:
            mock_get.return_value = mock_team_data
            
            result = await crud_team.get(mock_db, id=1)
    ...
    async def test_get_team_not_found(self, mock_db):
        ...
        with patch.object(crud_team, 'get') as mock_get:
            ...
            result = await crud_team.get(mock_db, id=999)
            
            assert result is None
    
    async def test_get_teams(self, mock_db, mock_team_data):
        ...
        with patch.object(crud_team, 'get_multi') as mock_get_multi:
            ...
            result = await crud_team.get_multi(mock_db)
            
            assert len(result) == 1
            assert result[0]["name"] == "Test Team"
    
    async def test_update_team(self, mock_db, mock_team_data):
        ...
        with patch.object(crud_team, 'update') as mock_update:
            ...
            result = await crud_team.update(mock_db, db_obj=mock_team_data, obj_in=update_data)
            
            assert result is not None
            assert result["total"] == 150
    
    async def test_delete_team(self, mock_db, mock_team_data):
        """Test deleting a team"""
        with patch.object(crud_team, 'remove') as mock_remove:
            mock_remove.return_value = mock_team_data
            
            result = await crud_team.remove(mock_db, id=1)
...
class TestTeamCheckpointLogic:
    ...
    async def test_add_checkpoint_timing_validation(self, mock_db, mock_team_data):
        ...
        with patch.object(crud_team, 'add_checkpoint') as mock_add_checkpoint:
            ...
            with pytest.raises(ValueError, match="Invalid checkpoint timing"):
                await crud_team.add_checkpoint(mock_db, team_id=1, checkpoint_data=checkpoint_data)
    
    async def test_add_checkpoint_order_validation(self, mock_db, mock_team_data):
    ...
    async def test_add_checkpoint_success(self, mock_db, mock_team_data):
        ...
        with patch.object(crud_team, 'add_checkpoint') as mock_add_checkpoint:
            ...
            result = await crud_team.add_checkpoint(mock_db, team_id=1, checkpoint_data=checkpoint_data)
            
            assert result is True
    
    async def test_add_checkpoint_random_cards(self, mock_db, mock_team_data):
...
class TestUserCRUD:
    ...
    async def test_create_user(self, mock_db, mock_user_data):
        ...
        with patch.object(crud_user, 'create') as mock_create:
            ...
            result = await crud_user.create(mock_db, obj_in=user_data)
            
            assert result is not None
            assert result["name"] == "Test User"
            assert result["email"] == "test@example.com"
    
    async def test_get_user(self, mock_db, mock_user_data):
        """Test getting a user"""
        with patch.object(crud_user, 'get') as mock_get:
            mock_get.return_value = mock_user_data
            
            result = await crud_user.get(mock_db, id=1)
            
            assert result is not None
            assert result["id"] == 1
            assert result["name"] == "Test User"
    
    async def test_get_users_by_team(self, mock_db, mock_user_data):
        ...
        with patch.object(crud_user, 'get_multi') as mock_get_multi:
            ...
            result = await crud_user.get_multi(mock_db, team_id=1)
