...
def _policy(
    ...
    profile: EventProfile,
    *,
    required: frozenset[Capability] | set[Capability] = frozenset(),
    forbidden: frozenset[Capability] | set[Capability] = frozenset(),
    description: str,
) -> EventPolicy:
...
POLICIES: dict[tuple[str, EventProfile], EventPolicy] = {
    ("peddy_paper", EventProfile.AUTONOMOUS): _policy(
        "peddy_paper",
    ...
    ("peddy_paper", EventProfile.GUIDED): _policy(
        ...
        required={
            ...
            Capability.CHECKPOINT_REDACTION,
            Capability.GUIDE_ARRIVAL,
            Capability.GUIDE_MODE,
        },
        forbidden=_PEDDY_FORBIDDEN,
        description="As equipas seguem o percurso com apoio de guias.",
    ),
    ("rally_tascas", EventProfile.STAFFED): _policy(
        "rally_tascas",
        EventProfile.STAFFED,
        required={Capability.STAFF_SCORING},
        forbidden={Capability.OLYMPIC_ROTATION},
        description="Rally com operações e avaliação por staff.",
    ),
    ("rally_tascas", EventProfile.SELF_CHECKIN): _policy(
        "rally_tascas",
        EventProfile.SELF_CHECKIN,
        required={Capability.QR_ARRIVAL},
        forbidden={Capability.OLYMPIC_ROTATION},
        description="Rally com check-in autónomo por QR onde a plataforma o suporta.",
    ),
    ("olympic", EventProfile.ROTATION): _policy(
        "olympic",
        EventProfile.ROTATION,
        required={Capability.OLYMPIC_ROTATION},
        forbidden={
            Capability.CHECKPOINT_REDACTION,
            ...
            Capability.HINTS,
            Capability.SKIP,
            Capability.DRINKING_SCORING,
        },
        description="Estações rodativas com resultados por prova.",
    ),
}
...
AVAILABLE_PROFILES: dict[str, frozenset[EventProfile]] = {
    ...
    "rally_tascas": frozenset(
        {EventProfile.CUSTOM, EventProfile.STAFFED, EventProfile.SELF_CHECKIN}
    ),
...
def default_profile(event_type: str) -> EventProfile:
    return {
        "peddy_paper": EventProfile.AUTONOMOUS,
        "rally_tascas": EventProfile.STAFFED,
        "olympic": EventProfile.ROTATION,
    }.get(event_type, EventProfile.CUSTOM)
...
def initial_settings_defaults(
    event_type: str, profile: str | EventProfile | None
) -> dict[str, object]:
...
def resolve_policy(event_type: str, profile: str | EventProfile | None) -> EventPolicy:
    ...
    return POLICIES.get(
        ...
        _policy(
            event_type,
            EventProfile.CUSTOM,
            description="Configuração livre, sujeita aos invariantes universais.",
        ),
    )
...
SETTING_CAPABILITIES: dict[Capability, tuple[str, bool]] = {
    ...
    Capability.QR_ARRIVAL: (
        "qr_checkin_enabled",
        False,
    ),  # event intent lives in config until a dedicated column exists
    Capability.GUIDE_ARRIVAL: ("guide_manual_arrival_enabled", False),
    Capability.HINTS: ("hints_enabled", False),
    Capability.SKIP: ("skip_enabled", False),
    Capability.PROXIMITY: ("proximity_enabled", False),
    Capability.COMPASS: ("compass_enabled", False),
    Capability.STAFF_SCORING: ("enable_staff_scoring", False),
    Capability.VERSUS: ("enable_versus", False),
    Capability.ROUTE_STAGES: ("route_stages_enabled", False),
    Capability.CHECKPOINT_HOURS: ("checkpoint_hours_enabled", False),
    Capability.LEG_TIME_SCORING: ("leg_time_scoring_enabled", False),
    Capability.GUIDE_MODE: ("guide_mode_enabled", False),
    Capability.BADGES: ("badges_enabled", False),
    Capability.DRINKING_SCORING: ("drinking_scoring", False),
}
...
DOMAIN_CONFIG_KEYS = frozenset({"qr_checkin_enabled", "drinking_scoring"})
