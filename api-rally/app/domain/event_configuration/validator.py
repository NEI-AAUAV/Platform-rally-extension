...
from .policies import Capability, CapabilityPolicy, EventProfile, SETTING_CAPABILITIES, resolve_policy
    ...
    def local_issues(*, event_type: str, profile: str | None, settings: object, config: dict | None = None) -> list[ConfigurationIssue]:
        ...
        for cap, value in caps.items():
            ...
            if value.policy is CapabilityPolicy.REQUIRED and not value.configured:
                issues.append(ConfigurationIssue("REQUIRED_CAPABILITY_DISABLED", ConfigurationIssueSeverity.ERROR, f"A capability obrigatória '{cap.value}' está desligada.", [SETTING_FIELDS.get(cap, cap.value)]))
            if value.policy is CapabilityPolicy.FORBIDDEN and value.configured:
                issues.append(ConfigurationIssue("FORBIDDEN_CAPABILITY_ENABLED", ConfigurationIssueSeverity.ERROR, f"A capability '{cap.value}' não está disponível neste perfil.", [SETTING_FIELDS.get(cap, cap.value)]))
...
SETTING_FIELDS = {capability: field for capability, (field, _inverted) in SETTING_CAPABILITIES.items()}
