...
describe("downloadEventResults", () => {
  ...
  it("throws when there is no staff session", async () => {
    ...
    await expect(downloadEventResults(1, "Rally")).rejects.toThrow(/administrador/i);
  });
  ...
  it("fetches the export endpoint with the staff bearer token", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/rally/v1/events/42/export", {
...
describe("downloadEventReport", () => {
  ...
  it("throws when there is no staff session", async () => {
    mockToken = null;
    const { downloadEventReport } = await import("@/services/eventExport");
    await expect(downloadEventReport(1, "Rally")).rejects.toThrow(/administrador/i);
  });

  it("fetches the report endpoint with the staff bearer token", async () => {
    ...
    const { downloadEventReport } = await import("@/services/eventExport");
    await downloadEventReport(42, "Rally 2026");

    expect(fetchMock).toHaveBeenCalledWith("/api/rally/v1/events/42/report", {
      headers: { Authorization: "Bearer staff-jwt" },
    });
  });
  ...
  it("triggers a download with a sanitized filename", async () => {
    ...
    const { downloadEventReport } = await import("@/services/eventExport");
    await downloadEventReport(7, "Rally / Edição #1");

    expect(anchor.download).toBe("Rally_Edição_1_relatorio.pdf");
    expect(anchor.click).toHaveBeenCalledOnce();
  });
  ...
  it("throws when the server responds with an error status", async () => {
    ...
    const { downloadEventReport } = await import("@/services/eventExport");
    await expect(downloadEventReport(1, "Rally")).rejects.toThrow(/500/);
  });
});
