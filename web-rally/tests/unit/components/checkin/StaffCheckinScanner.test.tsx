...
describe("StaffCheckinScanner", () => {
  ...
  it("closes the scanner when the check-in request fails (releases the camera)", async () => {
    h.staffCheckIn.mockRejectedValueOnce(
      new MockApiError(404, { detail: "Equipa não encontrada" }),
    );
  ...
  it("extracts the code from a raw ACCESS-CODE scan and submits it", async () => {
    ...
    render(<StaffCheckinScanner checkpointId={9} onTeamIdentified={onTeamIdentified} />, {
