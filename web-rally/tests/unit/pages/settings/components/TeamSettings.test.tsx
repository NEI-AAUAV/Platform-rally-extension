...
function Wrapper({ disabled, className }: { readonly disabled?: boolean; readonly className?: string }) {
  ...
  return (
    <FormProvider {...methods}>
      <form>
        <TeamSettings disabled={disabled} className={className} />
      </form>
    </FormProvider>
  );
}
...
describe('TeamSettings', () => {
  ...
  it('disables inputs and switch when disabled prop is true', () => {
    render(<Wrapper disabled />);
    expect(screen.getByLabelText('Número máximo de equipas')).toBeDisabled();
    expect(screen.getByLabelText('Máximo de membros por equipa')).toBeDisabled();
    expect(
      screen.getByLabelText('Ativar modo versus (competição entre equipas)'),
    ).toBeDisabled();
  });
