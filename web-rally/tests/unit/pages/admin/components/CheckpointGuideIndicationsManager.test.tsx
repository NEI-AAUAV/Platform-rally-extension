...
const {
  mockListGuideIndications,
  mockCreateGuideIndication,
  mockDeleteGuideIndication,
  mockToast,
} = vi.hoisted(() => ({
...
describe("CheckpointGuideIndicationsManager", () => {
  ...
  it("renders multiple indications without question/expected answer paragraphs when absent", async () => {
    mockListGuideIndications.mockResolvedValue({
      data: [indication({ id: 1, hint: "First hint" }), indication({ id: 2, hint: "Second hint" })],
    });
    renderWithClient();
  ...
  it("adds a new indication with hint, question and expected answer, resetting the form", async () => {
    ...
    await waitFor(() =>
      expect(
        (screen.getByPlaceholderText(/Indicação a dar à equipa/) as HTMLInputElement).value,
      ).toBe(""),
    );
  });
  ...
  it("deletes an indication and shows a success toast", async () => {
    ...
    await waitFor(() =>
      expect(mockDeleteGuideIndication).toHaveBeenCalledWith({ path: { indication_id: 42 } }),
    );
