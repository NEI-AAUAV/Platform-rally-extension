...
describe("BrandingSettings", () => {
  beforeEach(() => {
    ...
    mockUploadRallyBanner.mockResolvedValue({
      data: settings({ banner_url: "https://x/banner.png" }),
    });
    mockUploadRallyLogo.mockResolvedValue({ data: settings({ logo_url: "https://x/logo.png" }) });
    mockUploadRallyFavicon.mockResolvedValue({
      data: settings({ favicon_url: "https://x/favicon.png" }),
    });
  });
  ...
  it("saves branding text fields and shows a success toast", async () => {
    ...
    await waitFor(() =>
      expect(mockToast.success).toHaveBeenCalledWith("Identidade visual atualizada!"),
    );
  });
  ...
  it("uploads a banner image and shows a preview then success toast", async () => {
    ...
    await waitFor(() =>
      expect(mockToast.success).toHaveBeenCalledWith("Banner atualizado com sucesso!"),
    );
  });
  ...
  it("uploads a logo image and shows a success toast", async () => {
    ...
    await waitFor(() =>
      expect(mockToast.success).toHaveBeenCalledWith("Logótipo atualizado com sucesso!"),
    );
  });
  ...
  it("uploads a favicon image and shows a success toast", async () => {
    ...
    await waitFor(() =>
      expect(mockToast.success).toHaveBeenCalledWith("Favicon atualizado com sucesso!"),
    );
  });
  ...
  it("renders existing image urls for banner, logo and favicon", async () => {
    ...
    renderWithClient();
    expect(await screen.findByAltText("Banner atual")).toHaveAttribute(
      "src",
      "https://x/banner.png",
    );
