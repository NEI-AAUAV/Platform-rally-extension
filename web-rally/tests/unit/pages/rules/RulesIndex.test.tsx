...
describe('Rules index', () => {
  ...
  it('renders the "how it works" section open by default', () => {
    ...
    expect(
      screen.getByText(/Cada equipa percorre os postos do rally/),
    ).toBeInTheDocument();
  });
  ...
  it('reflects checkpoint order setting text', async () => {
    ...
    render(<Rules />);
    expect(screen.getByText('Os postos devem ser visitados pela ordem definida.')).toBeInTheDocument();
  });
  ...
  it('shows the "any order" text when checkpoint_order_matters is false', () => {
    ...
    render(<Rules />);
    expect(screen.getByText('Os postos podem ser visitados por qualquer ordem.')).toBeInTheDocument();
  });
