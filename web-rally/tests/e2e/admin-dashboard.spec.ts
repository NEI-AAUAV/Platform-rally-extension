...
async function mockEvaluations(page: Page, count: number) {
  await page.route('**/api/rally/v1/staff/all-evaluations', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({ evaluations: Array.from({ length: count }, (_, i) => ({ id: i + 1 })) }),
    }),
  );
}
...
test.describe('Admin dashboard', () => {
  ...
  test('renders per-checkpoint progress chart when checkpoints exist', async ({ page, context }) => {
    await mockSettings(page);
    await seedOidcSession(context, ADMIN_GROUPS);
