...
test.describe('Home', () => {
  ...
  test('shows LiveTop5 heading when score mode is not hidden and teams exist', async ({ page }) => {
    ...
    await mockTeams(page, [MOCK_TEAM, { ...MOCK_TEAM, id: 2, name: 'Second Team', classification: 2 }]);
    await mockCheckpoints(page);
  ...
  test('shows postos preview when show_checkpoint_map is true and checkpoints exist', async ({ page }) => {
