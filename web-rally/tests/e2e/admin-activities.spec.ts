...
test.describe("Admin activities", () => {
  test("create button is disabled with no checkpoints", async ({ page, context }) => {
    ...
    await expect(
      page.getByText("É necessário criar pelo menos um checkpoint antes de criar atividades."),
    ).toBeVisible();
    ...
    await page.route("**/api/rally/v1/activities/", (route) => {
      if (route.request().method() === "POST") {
        ...
        return route.fulfill({
          status: 200,
          contentType: "application/json",
          body: JSON.stringify(activity(capturedBody as Record<string, unknown>)),
        });
      }
      return route.fulfill({
        status: 200,
  ...
  test("editing an activity pre-fills the form", async ({ page, context }) => {
    ...
    await page
      .getByRole("button")
      .filter({ has: page.locator(".lucide-square-pen") })
      .first()
      .click();
    ...
    await page.route("**/api/rally/v1/activities/", (route) => {
      if (route.request().method() === "POST") {
        return route.fulfill({
          status: 400,
          contentType: "application/json",
          body: JSON.stringify({ detail: "Invalid activity type" }),
        });
      }
      return route.fulfill({
        status: 200,
