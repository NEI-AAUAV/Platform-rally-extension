...
async function mockVapidKey(page: Page) {
  // The notifications tab hides its form behind a FeatureDisabledAlert when
  // no VAPID key pair is configured (see admin/index.tsx) — mock one present
  // so these tests exercise the actual broadcast form.
  await page.route("**/api/rally/v1/push/vapid-public-key", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ public_key: "test-vapid-public-key" }),
    }),
  );
}

async function gotoNotifications(page: Page) {
  await mockVapidKey(page);
...
test.describe("Admin notifications (broadcast)", () => {
  test("sends a broadcast and shows the delivered count", async ({ page, context }) => {
    ...
    await page
      .getByPlaceholder("Ex: Vai começar a chover, procurem abrigo no posto mais próximo.")
      .fill("Abriguem-se no posto mais próximo.");
  ...
  test("shows an error when VAPID is not configured", async ({ page, context }) => {
    ...
    await page.route("**/api/rally/v1/push/broadcast", (route) =>
      route.fulfill({
        status: 503,
        contentType: "application/json",
        body: JSON.stringify({ detail: "Push not configured" }),
      }),
    );
