...
test.describe('Olympic-event rotation schedule', () => {
  ...
  test('generates a round-robin schedule for a 4-team olympic event with resolved team/checkpoint names', async ({
    ...
    await page.route('**/api/rally/v1/events', (route) =>
      route.fulfill({
        ...
        body: JSON.stringify([
          { id: 10, name: 'Jogos Olímpicos', event_type: 'olympic', description: null, start_time: null, end_time: null, is_current: true },
        ]),
      }),
    );
    ...
    await page.route('**/api/rally/v1/events/10/rotation-schedule', (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ rounds: ROUNDS }) }),
    );
  ...
  test('regenerating an existing schedule relabels the button and refetches', async ({
    ...
    await page.route('**/api/rally/v1/events/10/rotation-schedule', (route) => {
      generateCallCount += 1;
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ rounds: ROUNDS }) });
    });
  ...
  test('a generic (non-olympic) event never shows a rotation schedule button', async ({
    ...
    await page.route('**/api/rally/v1/events', (route) =>
      route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify([
          { id: 11, name: 'Rally Genérico', event_type: 'generic', description: null, start_time: null, end_time: null, is_current: true },
        ]),
      }),
    );
    await page.route('**/api/rally/v1/team/**', (route) =>
