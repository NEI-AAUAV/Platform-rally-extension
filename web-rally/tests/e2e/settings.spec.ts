...
import { MOCK_JWT_TOKEN_MANAGER, MOCK_RALLY_SETTINGS } from "../mocks/data";
...
test.describe("Settings", () => {
  ...
  test("should display settings sections", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText(
      /Equipas|Teams|Timing|Pontuação|Scoring|Display|Exibição/i,
      { timeout: 5000 },
    );
  });
  ...
  test("should handle save errors gracefully", async ({ page }) => {
    ...
    if (await saveButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await expect(page.locator("body")).toContainText(/erro|error|falha|failed/i, {
        timeout: 3000,
      });
    }
  });
  ...
  test("should validate form inputs", async ({ page }) => {
    ...
    if (await editButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      await editButton.click();
      await expect(page.locator('input[type="number"]:not([disabled])').first()).toBeVisible({
        timeout: 5000,
      });
    }
    ...
    if (count > 0) {
      ...
      if (await saveButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await expect(page.locator("body"))
          .toContainText(/inválido|invalid|deve|must/i, { timeout: 2000 })
          .catch(() => {
