...
import { MOCK_CHECKPOINT, MOCK_TEAM, MOCK_RALLY_SETTINGS, MOCK_ACTIVITY_LIST } from "../mocks/data";
...
test.describe("Admin Panel", () => {
  ...
  test("should switch between tabs", async ({ page }) => {
    ...
    await expect(page.getByText(MOCK_CHECKPOINT.name, { exact: false }))
    ...
    // Click on Activities tab (selecting a tab closes the mobile drawer, so
    // it needs reopening before the next nav interaction there)
    await openAdminNavIfMobile(page);
    ...
    await expect(page.locator("body")).toContainText(
      /Atividade|Activity|Nova|checkpoint|Checkpoint/i,
      { timeout: 5000 },
    );
  });
