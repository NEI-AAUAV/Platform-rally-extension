 ...
 */
export async function seedRealOidcSession(
  ...
  await context.addInitScript(
    ...
    [user.accessToken, user.sub, user.name, JSON.stringify(user.groups)] as [
      string,
      string,
      string,
      string,
...
export async function apiCall<T = unknown>(
  ...
  if (!response.ok) {
    throw new Error(
      `apiCall ${method} ${path} failed: ${response.status} ${await response.text()}`,
    );
  }
