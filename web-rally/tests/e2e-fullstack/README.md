...
exists only as a mocked spec — it isn't backend-contract-dependent the way
everything else here is, so it's left mocked-only. Promote it to this
directory if that changes — the seeding pattern in `helpers/seedRally.ts` and
