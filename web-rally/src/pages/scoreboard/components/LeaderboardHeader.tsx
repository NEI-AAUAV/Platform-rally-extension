...
export default function LeaderboardHeader({ leader, teamsCount, checkpointsCount }: LeaderboardHeaderProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {/* leader hero */}
      <div className="rally-bg-accent-soft col-span-2 flex flex-col justify-between rounded-2xl border border-border p-5 sm:row-span-2 sm:min-h-[220px]">
        <div>
          ...
          <div className="rally-display mt-3 text-3xl font-bold text-foreground sm:text-4xl">{leader.name}</div>
          {reached > 0 && (
      ...
      <div className="rounded-2xl border border-border bg-muted p-5">
        <div className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Equipas</div>
        <div className="rally-display mt-1 text-3xl font-bold text-foreground">{teamsCount}</div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-muted p-5">
        ...
        <div className="rally-display mt-1 text-3xl font-bold text-foreground">{checkpointsCount ?? leader.total}</div>
      </div>
    </div>
  );
}
